...
PATTERNING_MODES = ["Serial", "Parallel"]

# Common application files
APPLICATION_FILES = ["Si", "Si-ccs", "Si-multipass"]
...
class MillingSettingsWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Set default to 1 nA (1e-9)
        default_index = self.milling_current_combo.findData(1e-9)
        if default_index >= 0:
        ...
        for voltage in COMMON_MILLING_VOLTAGES:
            label = format_value(voltage, unit="V", precision=1)
            self.milling_voltage_combo.addItem(label, voltage)
        # Set default to 30 kV
        default_index = self.milling_voltage_combo.findData(30e3)  # Default to 30 keV
        if default_index >= 0:
    ...
    def get_settings(self) -> FibsemMillingSettings:
        ...
        self._settings.milling_current = self.milling_current_combo.currentData()  # Convert pA to A
    ...
    def update_from_settings(self, settings: FibsemMillingSettings):
        ...
        current_index = self.milling_voltage_combo.findData(settings.milling_voltage)
        if current_index >= 0:
            self.milling_voltage_combo.setCurrentIndex(current_index)
